...
def _attempt_generation(tier_name, generator, oin1_string, xyz_path, report):
    ...
    try:
        ...
        if atom_count_input != atom_count_generated:
            report["error"] = f"Atom count mismatch at {tier_name}. Input {atom_count_input} != Gen {atom_count_generated}"
            return False, last_gen_xyz_content
        ...
        report["error"] = f"Generation/Verification failed at {tier_name}: {type(e).__name__}: {e}\n{traceback.format_exc()}"
        return False, last_gen_xyz_content
    finally:
...
def main():
    parser = argparse.ArgumentParser(description="Dataset Roundtrip Testing (Multi-Pass Architecture)")
    parser.add_argument("--dataset-dir", type=str, default="../tmCAT-tmPHOTO_xyz_dataset", help="Path to dataset directory")
    ...
    if requires_mace:
        ...
        for i, (xyz_path, oin1_string, report) in enumerate(requires_mace, 1):
            ...
            success, last_xyz = _attempt_generation("MACE_1", gen_mace_1, oin1_string, xyz_path, report)
            if success:
                save_artifacts(report, last_xyz, output_dir, is_final=True)
                global_report.append(report)
                print("SUCCESS")
            ...
            success, last_xyz = _attempt_generation("MACE_5", gen_mace_5, oin1_string, xyz_path, report)
            if success:
                print("SUCCESS")
            else:
                report["status"] = "failed"
