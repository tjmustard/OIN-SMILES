...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'tests', 'integration')))
from verify_xyz_to_oin import get_examples

from oinsmiles.generator3d import embed, calculate_heavy_atom_rmsd
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark UFF pool sizes against unique conformers found.")
    parser.add_argument(
        "--oins", 
        nargs="*", 
        default=[],
        help="Additional OIN strings to test"
    )
    parser.add_argument(
        "--max-attempts", 
        type=int, 
        default=50,
        help="Number of iterations to generate structures"
    )
    ...
    for i, oin in enumerate(test_oins, 1):
        ...
        try:
            ...
            results.append(f"{display_oin:<50} | {found_str:<25} | {energy_str:<20} | {duration:<10.2f}")
