...
Both parts inherited their evidence from the Y2 wave (`docs/INJECTIVITY_Y2_FEASIBILITY.md`),
...
|---|---|---|
| P2 is total encoder blindness by default | `convert(R-BINAP)` and `convert(mirror)` byte-identical; `raw_equal = True`, `key_equal = True` | `docs/INJECTIVITY_Y1_P2_AXIAL.md` |
| the two atropisomers really are distinct | min proper-rotation mirror RMSD **4.02 Å** | same |
| RDKit does not perceive the axis from pure 3D | `FindPotentialStereo` returns 0 elements on the BINAP ligand; per the RDKit Book an atropisomer bond is marked only when a neighbour bond is *wedged* | `docs/INJECTIVITY_Y2_FEASIBILITY.md` |
...
| the stereogenicity gate is doing real work | suppresses **19 of 56** hindered axes (34%) as achiral; the independent geometric oracle agrees exactly — mirror RMSD **0.000 Å** on symmetric-end cases vs **2.80 Å** on the true atropisomer | `docs/INJECTIVITY_Y2_FEASIBILITY.md` |
...
1. **`docs/INJECTIVITY_Y2_FEASIBILITY.md` is stale on the multi-axis attribution.** It still
   ...
   both** (R1, R3). The corrected account lives in `docs/AXIAL_v0.4.5_LANE4.md` and
   `docs/KNOWN_LIMITATIONS.md` §3 of the axial section. Prefer those.
2. **`docs/INJECTIVITY_Y2_FEASIBILITY.md` also still carries the "gate ON by default"
...
4. **`docs/INJECTIVITY_Y1_P2_AXIAL.md` still describes the token's stereogenicity gate as
...
emitting nothing). `docs/AXIAL_v0.4.5_LANE4.md` §6 documents the re-run command
