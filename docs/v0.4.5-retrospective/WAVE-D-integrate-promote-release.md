...
2. **An earlier revision of the gate document said the key instability went "1 in 5 → 1 in 25", which
   ...
   i.e. **1 in 18.6 ≈ 1 in 19** — the figure `docs/CANONICAL_OIN_v0.4.5.md` carried correctly all
   along. `docs/PROMOTION_GATE_v0.4.5.md` §1 and §4 have since been corrected in place, with the
...
- Release notes: `docs/CANONICAL_OIN_v0.4.5.md`. Live state: `tools/v045_state.sh`, which computes
...
- **`OIN_BORON_CAGE` promoted to default-ON.** On the 936-molecule re-baseline, **34 of the 36
  ...
  **48/48 round-tripping** (`docs/BORON_CAGE_v0.4.5.md`). **The cost, recorded at the definition
...
| `05689832` | `docs/CANONICAL_OIN_v0.4.5.md` — the release notes |
...
- **Documents of record:** `docs/PROMOTION_GATE_v0.4.5.md` (the gate and its vetoes) ·
  `docs/CANONICAL_OIN_v0.4.5.md` (release notes) · `docs/V045_STATUS_2026-07-25.md` (mid-wave status,
  including every lane's numbers and the five-instrument-defects table) ·
  `docs/V046_HFAITHFUL_FINDINGS.md` (post-release refutations) · `docs/KNOWN_LIMITATIONS.md` ·
...
**A note on the instrument defects, because the pattern generalizes.** `docs/V045_STATUS_2026-07-25.md`
...
2. **Stop propagating "1 in 25"** if you meet it in an older copy or a downstream quote. The gate's
   ...
   `docs/PROMOTION_GATE_v0.4.5.md` §1 and §4 are already corrected, with the arithmetic shown.
...
10. **Where the remaining accuracy actually is, so the next wave is not spent on the notation.** Of
    ...
    roughly half of that is timeout-shaped. But `docs/V046_HFAITHFUL_FINDINGS.md` §3 then **refutes
