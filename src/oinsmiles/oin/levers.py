...
_HELD_OFF = {
    ...
    "OIN_ATTACH_CHECK": (
        ...
        "(docs/ATTACH_CHECK_v0.4.7.md §3): on the n=100 guard population it ERASES G1's failure "
    ...
    "OIN_BORON_GEN_FASTFAIL": (
        ...
        "tools/boron_gen_sweep_14silent.sh, full 48-molecule sample, not a partial one): the "
        "predicate flags EXACTLY the 40 molecules that burn the embed budget (0/40 of THOSE "
        "produce a structure) and flags NONE of the other 8 (1 confirmed success plus 7 already-"
        "instant failures: 4 unsupported-geometry, 3 uncoordinated-fragment) -- 48/48, zero "
        "mismatches. Every one of the 40 burns its whole embed budget (measured up to 2.3x OVER "
        "the requested cap; see the embed-time-budget finding below) before discovering it "
        "cannot assemble. Counter-attributed (tools/perf_attribute.py, not clock-only): the cost "
