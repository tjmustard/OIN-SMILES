...
_HELD_OFF = {
    ...
    "OIN_ACCEPT_SCORED": (
        ...
        "STILL OFF because the gate text says CORPUS and 22 stratified molecules is a cohort. "
        "Promoting on n=22 is the mistake this lane exists to avoid -- two fixtures produced four "
        "wrong answers about this same tail. The corpus A/B is a full sweep with the lever ON, "
        "which cannot run while a default-path sweep is in flight. Recommendation on the evidence: "
        "PROMOTE once that sweep confirms. Runtime numbers above were taken on a contended box "
        "(a 5k sweep running alongside), so read them as ratios within a run, not absolutes."
    ),
    "OIN_ETA_EARLY_EXIT": (
