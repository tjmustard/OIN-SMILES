...
class OINParser:
    ...
    def parse(self, oin_string: str) -> Tuple[str, Dict[str, str]]:
        """Parse an OIN string into the canonical SMILES and a dictionary of tags.
        ...
        if is_inline:
            ...
            # Reconstruct 'w' tag from vector_data list of SlotAssignment
            # Format: Rank.Idx:Slot;...
            # We assume atom index 0 for reconstruction as InlineHandler is lossy currently.
            # Winding is intentionally not propagated here (legacy w-tag path, out of scope).
            w_entries = []
            for sa in vector_data:
                w_entries.append(f"{sa.lig_rank}.0:{sa.slot}")
    ...
    def _parse_tags(self, oin_block: str) -> Dict[str, str]:
        """Parse the OIN tag block (deprecated).

        Parsing is now done in parse(); kept for compatibility.
        """
        return {}

    def parse_coordinates(self, tag_content: str) -> List[Tuple[int, float, float, float]]:
        """Parse the 'v' tag into a list of (idx, x, y, z) tuples.
    ...
    def parse_connectivity(self, tag_content: str) -> List[Tuple[int, int]]:
        """Parse the 'v' tag to extract connectivity.
