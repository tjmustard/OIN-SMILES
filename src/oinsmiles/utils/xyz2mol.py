...
from rdkit.Chem import AllChem, GetPeriodicTable, rdchem, rdEHTTools, rdmolops
import sys
...
def _align_to_pai(tmc_mol, xyz_coords, metal_idx):
    ...
    True implementation moved to oinsmiles.utils.oin_aligner._align_to_pai
    """
    from ..utils.oin_aligner import _align_to_pai as real_align
    return real_align(tmc_mol, xyz_coords, metal_idx)


def get_oin_string(tmc_mol, xyz_coords):
    """Generates the Open Isomer Notation (OIN) string for the molecule (V2.4).
    Delegates to oinsmiles.generation.canonicalizer.
    """
    return Canonicalizer.canonicalize(tmc_mol, xyz_coords)
