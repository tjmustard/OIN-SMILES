...
# REF: Molecular Vibrations (1955), the theory of infrared and raman vibrational spectra, Wilson
# Bright, J. C. Decius, Paul C. cross
def get_wilsonB_vector(xyz_coordinates, internal_coordinate):
    """Return the wilson b vector."""
    vector_dict = dict()
    if len(internal_coordinate) == 2:
        idx1, idx2 = internal_coordinate
        ...
        for idx in [idx1, idx2, idx3]:
            vector_dict[idx] = (
                sign_factor(idx, idx1, idx2) * uw / l_u + sign_factor(idx, idx3, idx2) * wv / l_v
            )
    else:
        idx1, idx2, idx3, idx4 = internal_coordinate
...
def get_wilsonB_derivative_sub_matrix(xyz_coordinates, internal_coordinate):
    """Return the wilson b derivative sub matrix."""
        ...
        for indices in scanning_indices:
            ...
            for i in range(3):
                for j in range(3):
                    ...
                    third_term = (
                        u_i * u_j + v_j * v_i - u_i * v_j * cos_q - kronecker_delta(i, j)
                    ) / (l_u * l_v * sin_q)
                    third_term *= sign_factor(idx1, a, b) * sign_factor(idx2, c, b)
                    fourth_term = (
                        v_i * v_j + u_j * u_i - v_i * u_j * cos_q - kronecker_delta(i, j)
                    ) / (l_u * l_v * sin_q)
        ...
        for indices in scanning_indices:
            ...
            for i in range(3):
                for j in range(3):
                    ...
                    fifth_term = (uw[i] * (w_j - 2 * u_j * cos_phi_u + w_j * cos_phi_u**2)) / (
                        2 * l_u * l_w * sin_phi_u**4
                    )
                    fifth_term *= sign_factor(idx1, a, b) * sign_factor(idx2, b, c) + sign_factor(
                        idx1, c, b
                    ) * sign_factor(idx2, b, a)
                    sixth_term = (uw[j] * (w_i - 2 * u_i * cos_phi_u + w_i * cos_phi_u**2)) / (
                        2 * l_u * l_w * sin_phi_u**4
                    )
                    sixth_term *= sign_factor(idx1, a, b) * sign_factor(idx2, b, c) + sign_factor(
                        idx1, c, b
                    ) * sign_factor(idx2, b, a)
                    seventh_term = (vw[i] * (w_j + 2 * u_j * cos_phi_v + w_j * cos_phi_v**2)) / (
                        2 * l_v * l_w * sin_phi_v**4
                    )
                    seventh_term *= sign_factor(idx1, d, c) * sign_factor(idx2, c, b) + sign_factor(
                        idx1, c, b
                    ) * sign_factor(idx2, d, c)
                    eighth_term = (vw[j] * (w_i + 2 * u_i * cos_phi_v + w_i * cos_phi_v**2)) / (
                        2 * l_v * l_w * sin_phi_v**4
                    )
                    eighth_term *= sign_factor(idx1, d, c) * sign_factor(idx2, c, b) + sign_factor(
                        idx1, c, b
                    ) * sign_factor(idx2, d, c)
                    ninth_term = (
                        uw[i]
                        * (u_j + u_j * cos_phi_u**2 - 3 * w_j * cos_phi_u + w_j * cos_phi_u**3)
                    ) / (2 * l_w**2 * sin_phi_u**4)
                    ninth_term *= sign_factor(idx1, b, c) * sign_factor(idx2, c, b)
                    tenth_term = (
                        uw[j]
                        * (u_i + u_i * cos_phi_u**2 - 3 * w_i * cos_phi_u + w_i * cos_phi_u**3)
                    ) / (2 * l_w**2 * sin_phi_u**4)
                    tenth_term *= sign_factor(idx1, b, c) * sign_factor(idx2, c, b)
                    eleventh_term = (
                        vw[i]
                        * (v_j + v_j * cos_phi_v**2 + 3 * w_j * cos_phi_v - w_j * cos_phi_v**3)
                    ) / (2 * l_w**2 * sin_phi_v**4)
                    eleventh_term *= sign_factor(idx1, b, c) * sign_factor(idx2, b, c)
                    twelvth_term = (
                        vw[j]
                        * (v_i + v_i * cos_phi_v**2 + 3 * w_i * cos_phi_v - w_i * cos_phi_v**3)
                    ) / (2 * l_w**2 * sin_phi_v**4)
...
def get_wilsonB_derivative_matrix(xyz_coordinates, internal_coordinates):
    """Return the wilson b derivative matrix."""
    n = len(xyz_coordinates)
    m = len(internal_coordinates)
...
def get_internal_coordinate_info(molecule, internal_coordinates):
    """Return the internal coordinate info."""
    infos = dict()
    coordinate_list = molecule.get_coordinate_list()
