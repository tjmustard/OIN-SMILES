...
**Suite after S1+S2+S3+S4:** `discover tests/unit` **304 OK** (skip=5 on rdkit 2026.03.3; skip=4 on 2025.09.3 — both blessed); `verify_xyz_to_oin.py` 26/27, the one failure being **VOacac2 E/Z (`/C=C(/C)`), pre-existing on `main` — not S4's**; lint clean. Note the handoffs' quoted baseline "233 OK / 4 skipped" is **stale** — pristine `main` in a fresh `uv sync` worktree was already 245 OK / 5 skipped before S1. Measure your own baseline before claiming a regression.

**`main` is 6 commits ahead of `origin/main` and UNPUSHED** (standing instruction). S1/S2 landed by local fast-forward, S3 and S4 by local squash-merge (`d96fd03`, `882cefb`) — reconcile before pushing. No CHANGELOG entry or pyproject bump for 0.3.6 yet (pyproject still `0.3.5`); treat that as a wave-end task, along with re-running the dataset on `882cefb` and regenerating `CASE_REGISTRY.md`.
