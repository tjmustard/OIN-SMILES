...
def propagate_blast_radius(yaml_path: str, dirty_node_ids: List[str]):
    ...
    while queue:
        ...
        # 1. Propagate up through 'implements' (Submodule -> Module)
        implements = current_node.get('implements')
        if isinstance(implements, str): implements = [implements]
        elif implements is None: implements = []
        
        for parent_id in implements:
            if parent_id in nodes and parent_id not in processed:
                nodes[parent_id]['status'] = 'needs_review'
                processed.add(parent_id)
                queue.append(parent_id)
                
        # 2. Propagate up through 'depends_on' (Dependency -> Dependant)
