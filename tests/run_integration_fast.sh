...
echo "Round-Trip Verification (Limit 16)..."
uv run python tests/integration/verify_roundtrip.py --limit 16 --output-dir "$ARTIFACTS_DIR" | tee "${ARTIFACTS_DIR}/roundtrip_log.txt"

echo "XYZ to OIN Verification (Limit 16)..."
uv run python tests/integration/verify_xyz_to_oin.py --limit 16 | tee "${ARTIFACTS_DIR}/integration_log.txt"
