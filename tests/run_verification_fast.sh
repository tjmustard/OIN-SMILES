...
show_help() {
    echo "Usage: bash tests/run_verification_fast.sh [OPTIONS]"
    echo ""
    echo "Runs fast integration tests (limit 4 examples)."
    echo ""
    echo "Options:"
...
eval uv run python tests/integration/verify_xyz_to_oin.py --limit 4 --output-dir \"$ARTIFACTS_DIR\" $ONLY_ARG \| tee \"${ARTIFACTS_DIR}/integration_log.txt\"
...
eval uv run python tests/integration/verify_roundtrip.py --limit 4 --output-dir \"$ARTIFACTS_DIR\" $ONLY_ARG $EXTRA_ARGS \| tee \"${ARTIFACTS_DIR}/roundtrip_log.txt\"
