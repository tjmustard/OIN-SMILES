"""N-chiral stereocenter encoding tests.

Tests N-centered stereochemistry encoding for the RR-BDNN (R,R-bis(diethylamino)
naphthalene) ligand — a diphosphine with two independent N stereocenters.
...
class TestChiralN(unittest.TestCase):
    """Verify N-stereocenter encoding correctness."""

    def test_n_stability_roundtrip(self):
        """RR-BDNN round-trip: XYZ → OIN → XYZ → OIN (oin == oin')."""
        xyz_path = get_fixture_path("PdCl2-RR-BDNN.xyz")
    ...
    def test_n_cip_oracle(self):
        """RR-BDNN: OIN encoding is stable and verifiable via RDKit parsing."""
        xyz_path = get_fixture_path("PdCl2-RR-BDNN.xyz")
        ...
        # Check that the OIN contains N atom markers (either [N or N{})
        self.assertTrue("[N" in oin or "N{" in oin, "OIN should contain N atom markers")
